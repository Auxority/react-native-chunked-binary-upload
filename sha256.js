const sha256 = (input) => {
    ...
    let message = new Uint8Array([...input, 0x80]);
    let messageLength = input.length * 8;

    while ((message.length + 8) % 64 !== 0) {
        message = new Uint8Array([...message, 0x00]);
    }

    // Append the length of the original message as a 64-bit big-endian integer
    const lengthBytes = new Array(8).fill(0);
    for (let i = 7; i >= 0; i--) {
        lengthBytes[i] = messageLength & 0xff;
        messageLength >>>= 8;
    }
    message = new Uint8Array([...message, ...lengthBytes]);
    ...
    const hashArray = Array.from(h, num => num.toString(16).padStart(8, '0'));
    return hashArray.join('');
};

const processChunk = (chunk, hash) => {
